...
test.describe('Sauce Demo Burger Menu', () => {
    ...
    test.describe('menu navigation', () => {
        ...
        test('should reset app state completely', async ({ page }) => {
            ...
            await expect(firstProduct.locator('[data-test="add-to-cart-sauce-labs-backpack"]')).toBeVisible();
        });
        ...
        test('should prevent menu interaction during transitions', async ({ page }) => {
            // Click menu button and wait for animation
            await page.locator('#react-burger-menu-btn').waitFor({ state: 'visible' });
            await page.click('#react-burger-menu-btn');
            await page.locator('.bm-menu').waitFor({ state: 'visible' });
            await page.waitForTimeout(300); // Wait for animation
            
            // Menu should be fully visible and interactive
            await expect(page.locator('.bm-menu')).toBeVisible();
            
            // Try to click menu button again - verify it's not clickable
            const menuBtn = page.locator('#react-burger-menu-btn');
            try {
                // This click should either fail or be ignored
                await menuBtn.click({ timeout: 1000 });
            } catch (e) {
                // Expected - button should not be clickable
            }
            ...
            await page.locator('.bm-menu').waitFor({ state: 'hidden', timeout: 5000 });
            await expect(page.locator('.bm-menu')).not.toBeVisible();
        });
